            ...
            <Field label={`Exposed inputs (${doc.bindings.length} detected)`}>
              <div className="divide-y divide-line rounded-xl border border-line">
                {doc.bindings.map((b, i) => (
                  <div key={b.key} className="flex items-center gap-3 px-3 py-2">
                    <Input
                      value={b.label}
                    ...
                    />
                    <Badge>{b.type}</Badge>
                    <span className="w-28 truncate font-mono text-[10.5px] text-fg-3">
                      #{b.node}.{b.input}
                    </span>
...
export function SkillsPage(): React.JSX.Element {
  ...
  return (
    <Page>
      {/* Hero carousel */}
      <div className="relative h-[330px] overflow-hidden border-b border-line">
        <AnimatePresence mode="sync">
          <motion.div key={f.id} className="absolute inset-0" initial={{ opacity: 0, scale: 1.04 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.9, ease }}>
            <SkillArt colors={f.art} icon={f.icon} className="absolute inset-y-0 right-0 w-[70%] opacity-80" />
            <div className="absolute inset-0 bg-gradient-to-r from-[var(--panel-solid)] via-[color-mix(in_oklab,var(--panel-solid)_85%,transparent)] to-transparent" />
          </motion.div>
        </AnimatePresence>
        <div className="relative z-10 flex h-full flex-col justify-center gap-5 px-10">
          <h1 className="display text-[34px] uppercase">Explore skills</h1>
          <ul className="space-y-2 text-[13px] text-fg-3">
            <li className="flex items-center gap-2.5">
              <Zap className="size-3.5 text-fg-2" />
              <span>
                <b className="font-semibold text-fg">A skill</b> is one ready-made ability, like locking a character
              </span>
            </li>
            <li className="flex items-center gap-2.5">
              <FileJson className="size-3.5 text-fg-2" />
              <span>
                <b className="font-semibold text-fg">Any ComfyUI workflow</b> can become a one-click skill
              </span>
            </li>
            <li className="flex items-center gap-2.5">
              <Rocket className="size-3.5 text-fg-2" />
              <span>
        ...
        <div className="absolute right-8 bottom-6 z-10 flex items-end gap-4 text-right">
          <div>
            <div className="text-[15px] font-semibold">{f.name}</div>
            <div className="text-[12px] text-fg-2">{f.blurb}</div>
            <button onClick={() => void f.run(navigate)} className="mt-2 inline-flex items-center gap-1 rounded-full bg-black/40 px-3 py-1 text-[12px] font-medium backdrop-blur hover:bg-black/60">
              Open <ChevronRight className="size-3.5" />
            </button>
          </div>
        </div>
        <div className="absolute top-5 right-8 z-10 flex gap-1.5">
          {featured.map((s, i) => (
        ...
        <button onClick={() => setSlide((s) => (s - 1 + featured.length) % featured.length)} className="absolute top-1/2 left-[46%] z-10 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur transition hover:bg-black/55">
          <ChevronLeft className="size-4" />
        </button>
        <button onClick={() => setSlide((s) => (s + 1) % featured.length)} className="absolute top-1/2 right-6 z-10 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur transition hover:bg-black/55">
        ...
        <AnimatePresence mode="wait">
          {tab === 'explore' ? (
            <motion.div key="explore" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease }}>
              <div className="mt-8 mb-4 flex items-center gap-2 text-[15px] font-bold tracking-tight uppercase">
                <span className="grid size-5 place-items-center rounded-full bg-grad text-white">
                  <Zap className="size-3" />
                </span>
                Skills by Stitch
              </div>
              <motion.div variants={stagger(0.04)} initial="initial" animate="animate" className="grid grid-cols-3 gap-4">
                {list.map((s) => (
                  <motion.button key={s.id} variants={rise} whileHover={{ y: -4 }} transition={spring} onClick={() => void s.run(navigate)} className="group text-left">
                    <SkillArt colors={s.art} icon={s.icon} className="aspect-[16/9] rounded-2xl ring-1 ring-line transition-transform duration-500" />
                    <div className="mt-3 flex items-center gap-2.5">
                      <span className="grid size-7 place-items-center rounded-lg border border-line bg-white/[0.05] text-fg-2 [&>svg]:size-3.5">{s.icon}</span>
                  ...
                  {mine.map((s) => (
                    <div key={s.id} className="glass hairline group rounded-2xl p-4">
                      ...
                      <div className="mt-4 flex items-center gap-2">
                        ...
                        <Button size="xs" variant="ghost" icon={<Trash2 className="size-3" />} onClick={() => void db.remove('skills', s.id).then(() => useGen.getState().refreshRecipes())}>
                          Remove
                        </Button>
                        <Button size="sm" variant="primary" icon={<Play className="size-3" />} onClick={() => setRunning(s)}>
                          Run
                        </Button>
                      </div>
                    </div>
